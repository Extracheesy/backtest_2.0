...
class TechnicalAnalysis:
        ...
        if isinstance(price, int) or isinstance(price, float):
            ...
            if len(df) > 0:
                ...
                if float(df_last[0]) < price:
                    print(f"Support level of {str(df_last[0])} formed at {str(df_last.index[0])}", self.app, "normal")
                elif float(df_last[0]) > price:
                    print(f"Resistance level of {str(df_last[0])} formed at {str(df_last.index[0])}", self.app, "normal")
                else:
                    print(f"Support/Resistance level of {str(df_last[0])} formed at {str(df_last.index[0])}", self.app, "normal")
    ...
    def print_support_resistance_levels_v2(self, price: float = 0) -> None:
        if isinstance(price, int) or isinstance(price, float):
            ...
            print(f"Support level is {str(round(support, 4))} and Resistance level is {str(round(resistance, 4))}", self.app, "normal")
    ...
    def save_csv(self, filename: str = "tradingdata.csv") -> None:
        ...
        try:
            ...
            print(f"Unable to save: {filename}", self.app, "critical")
